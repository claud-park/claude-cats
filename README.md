...
> 원래 이름은 Claude Cats. 강아지 컨셉이 생기며 Claude Pets 로 개명(내부 식별자·번들
> ID `com.claudecats.app`·경로는 그대로 둔다 — 표시명·저장소 이름만 바꿈).

실행 중인 Claude Code 세션을 바탕화면 위(아이콘 아래) 동물(고양이·강아지)로 보여주는 macOS 앱.
