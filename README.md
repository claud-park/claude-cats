...
설치를 누르면 `scripts/install.sh --self-update <설치 디렉터리> <지금 pid>` 가 돈다.
순서가 전부다.

1. 작업 트리가 더러우면 **여기서 멈춘다**
2. `git pull --ff-only origin <브랜치>` — fast-forward 가 아니면 멈춘다
3. `./scripts/bundle.sh` 로 다시 빌드
4. 지금 돌고 있는 자리(`/Applications` 든 `dist/` 든)에 복사
5. `open -n` 으로 새 인스턴스를 띄우고, **그게 실제로 뜬 걸 확인한 다음에야** 기존 pid 에 `kill`

3~5 중 어디서 실패해도 non-zero 로 끝나고 쓰던 앱은 그대로 남는다. 실패한 업데이트가 앱을
없애 버리는 게 제일 나쁜 결과라, 기존 인스턴스는 항상 마지막에 내린다.
